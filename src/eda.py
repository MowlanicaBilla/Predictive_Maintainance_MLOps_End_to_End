...
from config.config import logger, ARTIFACTS_DIR
...
# def question_four_point_two(df):
#     # Filter data for instances with machine failures
#     failed_instances = df  # Include both instances with Machine failure as 0 and 1

#     variables_for_analysis = ['Air temperature [K]',
#        'Process temperature [K]', 'Rotational speed [rpm]', 'Torque [Nm]',
#        'Tool wear [min]', 'Machine failure']

#     # Create a Parallel Coordinates Plot
#     fig = px.parallel_coordinates(failed_instances[variables_for_analysis], color='Machine failure',
#                               labels={'Machine failure': 'Machine Failure'},
#                               color_continuous_scale=px.colors.diverging.Tealrose, 
#                               title='Parallel Coordinates Plot for Machine Failures')

#     fig.update_traces(text=variables_for_analysis, textposition='auto')
#     plot_json = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
#     logger.info("EDA Question 4.2 complete")
#     return plot_json
...
#     fig = px.scatter(data_frame=df, x='Air temperature [K]', y='Process temperature [K]', color='Machine failure',
#                  color_continuous_scale=px.colors.diverging.Tealrose,
#                  title='Scatter Plot of Air and Process Temperature with Machine Failure')
#     plot_json = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
#     logger.info("EDA Question 7 complete")
#     return plot_json

# def question_eight(df):
#     fig = px.scatter(df, x='Torque [Nm]', y='Rotational speed [rpm]', color='Machine failure',
#                  color_continuous_scale=px.colors.diverging.Tealrose,
#                  title='Scatter Plot of Torque and Rotational speed with Machine Failure')
#     plot_json = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
#     logger.info("EDA Question 8 complete")
#     return plot_json
